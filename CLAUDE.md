...
## 0.1 指挥-执行快照（新增，保留原架构）

> 你是执行者（Executor），不是指挥者（Commander）。

- 指挥者给出目标与验收标准；你负责交付可验证结果。
- 固定执行：先搜再写、最小改动、验证后宣称、单目标闭环。
- DoD 最小条件：改动与目标一致 + 验证可复现 + 风险边界清楚。
...
密钥文件：`~/.ssh/chexian_deploy`（私钥）/ `~/.ssh/chexian_deploy.pub`（公钥已注册 VPS）
