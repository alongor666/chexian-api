...
1. **已有实现？** → `grep -r "关键词" src/ server/src/`，禁止假设"不存在"
2. **改多少行？** → 改动 < 30% 用 `Edit`（只发 diff）；≥ 50% 才用 `Write`（全文覆盖）
3. **怎么验证？** → 确定验证命令，执行后**必须贴出结果**（HTTP 状态/数据行数/错误信息）

### 执行纪律

| 红线 | 正确做法 |
|------|---------|
| **执行不规划** | `commit/push/PR` → 直接执行命令，零分析零摘要 |
| **先搜再写** | 写代码前 `grep/glob` 搜索整个项目，禁止假设"不存在" |
| **验证不声称** | 禁止声称"已可用"，必须通过真实 API 请求验证并贴出结果 |
| **修补不拆除** | 安全加固/重构禁止删除整个模块/插件，只能修补漏洞 |
| **并行不串行** | 3+ 独立模块/任务必须用并行 sub-agents（Task 工具） |
| **层级不扁平** | "下钻"需求先确认交互模型：面包屑 vs 扁平筛选 |
| **聚焦不发散** | 每次会话专注一个目标，完成并验证后再接下一个 |
| **Edit > Write** | 改动 < 30% 用 Edit，≥ 50% 才用 Write，新建文件用 Write |
...
- 禁止以“之前已部署过本地版本”为由跳过最终 merge 版本的重新发布。

**硬规则 3：发布脚本健康检查规则**
- 所有 VPS 发布脚本禁止只用固定 `sleep N` 作为健康判断。
- 必须使用“轮询重试 + 超时退出”检查 `health` 或等价端点，覆盖 DuckDB 冷启动与 PM2 重启竞争窗口。

**硬规则 4：证据分级**
- 本地绿灯：`bun run governance`、`bun run test -- --run`、`bun run typecheck`、`cd server && bun run build`
- 服务绿灯：根地址 `/` 与 `/health`
- 业务绿灯：登录 + 至少一个核心业务 API
- 页面绿灯：至少一个真实浏览器场景

**执行要求**：上述 4 级证据必须出现在输出中；任一级缺失，都不得写“已完成部署”。
...
### 数据同步护栏（RED LINE - 2026-03-13 起强制执行）

> **背景**：VPS `current/` 积累了 3 个时间段重叠的 Parquet，`UNION ALL` 后数据翻倍（1,837,252行 vs 正确的 1,161,809行）。

❌ **禁止在 `current/` 目录中保留时间范围重叠的 Parquet 文件**

| 违规示例 | 后果 |
|---------|------|
| `历史数据_20231101_20241231.parquet` + `每日数据_20231101_20260310.parquet` | 重叠区间数据翻倍 |

**正确做法**：多文件日期区间必须**互补**（首尾相接，无重叠），治理脚本第 13 项自动检测。

**同步命令**（默认清理，防止叠加）：
```bash
node scripts/sync-vps.mjs              # 默认：清理 current/ 后同步（防重叠）
node scripts/sync-vps.mjs --dry-run    # 仅打印执行计划，不连接 VPS
node scripts/sync-vps.mjs --export     # 预聚合模式（推荐新增维度时使用）
node scripts/sync-vps.mjs --check      # 仅预检 SSH 与待同步文件
node scripts/sync-vps.mjs --no-restart # 同步但不重启 PM2
node scripts/sync-vps.mjs --keep-old   # ⚠️ 危险！会导致重复数据
```
...
| `server/src/config/preset-users.ts` | 预设用户定义（角色/路由权限/密码哈希，从 auth.ts 拆出） |
| `server/src/config/capability-registry.ts` | AI 能力注册表（对话式导航的能力 ID 映射，247行） |
| `server/src/services/requirement-detector.ts` | 自然语言→能力 ID 识别服务 |
| `server/src/utils/parquet-source.ts` | Parquet 文件来源自动检测 |
| `server/src/services/access-control.ts` | 细粒度路由权限控制 |
...
| `/api/data/*` | 文件上传、列表、加载 |
| `/api/ai/*` | NL2SQL、智能分析、`POST /api/ai/detect-requirement`（对话式需求识别） |
...
bun run test:e2e                # E2E 全套（Playwright，需先启动 dev:full）
bun run test:e2e:ui             # Playwright 交互式调试模式
bun run test:e2e:cleanup-gate  # 清理零故障门禁回归
bun run governance              # 治理校验
# E2E 首次使用：bunx playwright install chromium
# E2E 默认凭据：admin / CxAdmin@2026!（可用 E2E_USERNAME/E2E_PASSWORD 覆盖）
```
...
- 首页 AI 对话式导航（`AIAssistantPage` → `POST /api/ai/detect-requirement` → `capability-registry` → 路由跳转）
...
- 声称“网站访问正常/生产已恢复”前，必须额外展示 `/`、`/health`、登录、至少一个核心业务 API、至少一个真实浏览器场景这 5 项证据
- 若发布期间发生新的 `commit/push/rebase/merge`，必须基于最终远端 HEAD 重新发布并重新验收，禁止复用旧部署证据
...
**数据同步**：`./scripts/sync-vps.mjs` 一键上传最新 Parquet → PM2 重启 → 健康检查。
...
完整变更历史见 [开发文档/CHANGELOG.md](./开发文档/CHANGELOG.md)。
