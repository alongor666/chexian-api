...
export const TonnageRoseChart: React.FC<TonnageRoseChartProps> = ({
  ...
  const content = (
    <>
      {showTitle && (
        <h3 className={`text-lg font-bold mb-4 text-center ${colorClasses.text.neutralBlack}`}>{title}</h3>
      )}

      {/* 指标切换器 */}
      <div className="flex justify-center items-center space-x-4 mb-4">
        <span className={`font-semibold text-sm ${colorClasses.text.neutral}`}>显示指标：</span>
