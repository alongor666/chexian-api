...
export interface MotoCostInputs {
  // 管理成本
  laborBaseRate: number;          // 人力成本基数 (%)
  fixedOperationRate: number;     // 固定运营成本率 (%)

  // 摩意险保费配比计算因子
  carAveragePremium: number;      // 摩托车单均保费 (元)
  motoAveragePremium: number;     // 摩意险件均保费 (元)
  motoQuantity: number;           // 摩意险份数

  // 车险参数
  carPremium: number;             // 车险保费 (万元)
  carLossRatio: number;           // 车险赔付率 (%)
  carHandlingFeeRate: number;     // 车险手续费率 (%)
  carSalesPromotionRate: number;  // 车险销推费用率 (%)
  carStandardPremiumRatio: number; // 车险标保系数

  // 摩意险参数
  motoPremium: number;            // 摩意险保费 (万元)
  motoLossRatio: number;          // 摩意险赔付率 (%)
  motoWithCarFeeRate: number;     // 随车业务费用率 (%)
  motoCardFeeRate: number;        // 卡单费用率 (%)
  motoSalesPromotionRate: number; // 摩意险销推费用率 (%)
  motoStandardPremiumRatio: number; // 摩意险标保系数
}
