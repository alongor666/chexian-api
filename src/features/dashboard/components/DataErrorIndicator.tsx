...
export function DataErrorIndicator({
  ...
  return (
    <div className={cn('rounded-lg p-4 mb-4', colorClasses.bg.danger, colorClasses.border.danger)}>
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3">
          <AlertTriangle className={cn('w-5 h-5 mt-0.5 flex-shrink-0', colorClasses.text.dangerLight)} />
          <div className="flex-1">
            <h3 className={cn('text-sm font-medium', colorClasses.text.dangerDark)}>数据加载失败</h3>
            <div className={cn('mt-2 text-sm', colorClasses.text.danger)}>
                ...
                {errorEntries.map(([key, error]) => (
                  <li key={key} className="flex items-center justify-between">
                    <span>
                      ...
                      {error.retryable && (
                        <span className={cn('ml-1', colorClasses.text.dangerLight)}>（可重试）</span>
                      )}
                    </span>
                    <button
                      onClick={() => onDismiss(key)}
                      className={cn('ml-2 p-1 rounded', colorClasses.bg.dangerHover)}
                      title="忽略此错误"
                    >
                      <X className="w-3 h-3" />
                    </button>
            ...
            <div className="mt-3 flex gap-2">
              <button
                ...
                className={cn(
                  ...
                  colorClasses.bg.dangerSolid,
                  colorClasses.bg.dangerHover,
              ...
              {errorEntries.length > 1 && (
                <button
                  ...
                  className={cn(
                    ...
                    'bg-white', colorClasses.bg.dangerHover, colorClasses.text.danger, colorClasses.border.danger
                  )}
                >
                  全部忽略
                </button>
...
export function InlineDataError({
  ...
  return (
    <div className={cn('flex items-center justify-center h-full rounded-md p-4', colorClasses.bg.danger)}>
      <div className="text-center">
        <AlertTriangle className={cn('w-8 h-8 mx-auto mb-2', colorClasses.text.dangerLight)} />
        <p className={cn('text-sm mb-2', colorClasses.text.danger)}>
          {label}加载失败
        </p>
        <p className={cn('text-xs mb-3', colorClasses.text.dangerLight)}>{error.message}</p>
