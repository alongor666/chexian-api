...
export const FRAMEWORK = {
  ...
  action: {
    title: '动作层',
    desc: '怎么干',
    items: ['加码 · 复制', '优化 · 整改', '暂停 · 退出', '（每张图卡片标注对应动作）'],
  },
} as const;
...
export const CARD_META: Record<string, LedgerCardMeta> = {
  ...
  'chart-06': {
    ...
    name: '赔款发展三角图',
    usage:
      '行 = 起保年度（同一年起保的保单批次），列 = 发展月（满 3/6/9 月…）。只有对角线以上的格子有数据，横向对比同一发展期不同年度的赔付率，判断赔款发展是否在提速。',
  ...
  'chart-11': {
    ...
    name: '变动成本率控制图',
    usage:
      '中心线 = 历史均值，上 / 下控制限 = 中心线 ± 2σ。落在控制限内是"正常波动"；一旦突破，才是真正需要介入的信号——避免把噪音当问题、把问题当噪音。',
    note: '真实数据 · 周序号 × 变动成本率(%)；控制限 = 中心线 ± 2σ 规则',
    action: '预警',
