...
| 2026-03-10 | AI 注释联动与长表平移收口完成 (B227) | 对照原计划里的“AI 解读强化”与“长页面一致体验”，让 `cross-sell` AI 解读与两张趋势图共享同一批极值注释数据，并把 `growth/cost` 核心长表统一接入 sticky/frozen 容器；同时移除未稳定的热力图去重 E2E 草稿 | 代码证据：`src/features/dashboard/CrossSellAnalysisPanel.tsx`、`src/features/dashboard/CrossSellTrendChart.tsx`、`src/features/growth/components/GrowthDetailSection.tsx`、`src/features/growth/components/GrowthComparisonSection.tsx`、`src/features/growth/components/ComparisonAnalysisPanel.tsx`、`src/features/cost/components/NewEarnedPremiumTable.tsx`、`tests/cross-sell-trend-annotations.test.ts`；验证证据：`bun run test -- --run tests/cross-sell-trend-annotations.test.ts tests/cross-sell-ux-review-fixes.test.tsx`、`bun run typecheck`、`bun run build`、`bun run governance`、`bun run test:e2e tests/e2e/05-cross-sell-ux.spec.ts tests/e2e/06-page-shell-ux.spec.ts tests/e2e/08-cross-sell-yearly-guard.spec.ts` |
