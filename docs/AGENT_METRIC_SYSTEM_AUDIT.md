...
底层指标注册表存在某个成本指标，不等于 Agent 可以输出财务利润、法定承保利润或审计盈亏结论。Agent 层必须逐项登记真实 metric id 的支持级别：
...
- `earned_margin_amount`、`projected_margin_amount`：纠正为 `supported`。边际贡献额仅扣除变动成本（赔付+费用），不等于承保利润、财务利润、净利润或财务盈亏。
- `earned_profit_amount`：保持 `unsupported`，因为它包含固定成本并直接进入利润语义。
...
4. 阶段 4：经营巡检聚合能力，聚合已完成的确定性诊断，不新增 SQL 或 LLM。
5. 阶段 4.5：确定性 Agent 能力总验收与 readiness hardening，明确 Stage 5 前置证据缺口。
6. 阶段 4.6：生产观测与验收证据闭环，统计 Agent 诊断审计日志、30 天错误率和调用方展示证据状态。
7. 阶段 4.9：确定性经营利润预测，不接 LLM、不访问 DuckDB、不生成 SQL。
8. 阶段 5：LLM 解释层，只解释工具返回数据，不生成指标。
9. 阶段 6：经营工作台与反馈复盘。
