...
`/api/agent/audit/readiness` 当前应显示 `currentStage=stage_4_8_display_contract_ready`，`readyForLlm=false`。Stage 5 仍需单独 PR 显式启动；B270 只定义 LLM 解释层边界，不会注册 `POST /api/agent/explain/diagnosis` 或放开 LLM 开关。在本阶段 readiness 只继续将以下生产运行证据缺口作为 LLM 前置阻塞项：
