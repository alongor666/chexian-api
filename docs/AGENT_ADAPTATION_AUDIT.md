...
Stage 4.7 新增 `scripts/chexian-verify-agent-production-smoke.mjs`，用于把 Stage 5 前置条件变成可执行验收命令。该脚本只调用固定 Agent API：7 个确定性诊断端点、`/api/agent/forecast/profit-scenario`、`/api/agent/explain/diagnosis`、`/api/agent/audit/observability` 和 `/api/agent/audit/readiness`；请求时间参数必须显式传入，报告中不写入 bearer token。脚本会分别记录诊断接口可用性、解释入口响应合同、调用方是否收到 `warnings` / `forbiddenInterpretations`、observability 状态和 readiness 中的 Stage 5 阻塞项。
...
Stage 4.9 新增 `/api/agent/forecast/profit-scenario`，用于执行 `forecast_operating_profit_scenario` 的确定性经营利润预测。该接口只做纯算术情景计算，不访问 DuckDB，不生成 SQL，不调用 LLM；输入必须显式提供终极变动成本率、终极固定成本率和已赚率计划。输出是经营预测结果，不是财务报表利润、法定承保利润或审计利润。
...
Stage 5A 新增 `/api/agent/explain/diagnosis`，用于解释调用方已取得的确定性诊断 API 响应。该接口不触发 DuckDB 查询、不调用 SQL 生成器、不执行工作流或任意工具；请求和响应均通过 Agent explanation schema Zod 校验。解释前先通过 `routeAgentQuestion` 识别 unsupported/caution 语义，并复用 `unsupportedMetricRegistry`、Agent 指标注册表、输入诊断结果的 `warnings` / `forbiddenInterpretations` 和 LLM `sql-guard`。生产 smoke harness 已加入固定 explanation 请求，并单独校验 `status`、`warnings` 与 `forbiddenInterpretations` 响应合同。本阶段仍不做聊天窗口，且 `/api/agent/audit/readiness` 继续保持 `readyForLlm=false`，等待解释入口生产 smoke 与观测证据闭环后再单独释放。
...
- `POST /api/agent/forecast/profit-scenario`：确定性经营利润预测情景计算。

所有返回结构都经过 Zod schema 校验。路由挂载在 `/api/agent/audit`，继续使用全局审计中间件，并在路由内使用 `authMiddleware` 和 `permissionMiddleware`。

`/api/agent/audit/readiness` 当前应显示 `currentStage=stage_4_9_deterministic_profit_forecast`，`readyForLlm=false`。Stage 5A 已注册 `POST /api/agent/explain/diagnosis`，但 readiness 开关仍需单独生产释放；forecast calculator 属于 Stage 4.9 确定性计算层，不改变 LLM 释放状态。在本阶段 readiness 只继续将以下生产运行证据缺口作为 LLM 前置阻塞项：

- 生产 audit log 能看到 `/api/agent/diagnosis/*` 调用记录。
- 最近 30 天 `/api/agent/diagnosis/*` error rate < 1%。

调用方展示证据已由 `scripts/chexian-verify-agent-production-smoke.mjs` 和 `tests/api/agent-production-smoke-harness.test.mjs` 锁定。底层确定性诊断接口和 explanation 入口必须继续返回 `warnings` 与 `forbiddenInterpretations`，调用方不得忽略这些字段。
...
`/api/agent/audit/readiness` 在确定性 forecast 能力完成后显示 `currentStage=stage_4_9_deterministic_profit_forecast`，但 `readyForLlm=false` 仍保持不变。
...
该命令会在 `output/agent-smoke/` 生成 JSON 报告。报告只用于验收确定性接口、explanation 入口响应合同和 Stage 5 前置证据，不会触发 NL2SQL 或自由 SQL。

2026-04-27 的线上 smoke 验收已确认 7 个确定性诊断端点、observability 和 readiness 均返回 200 JSON；调用方展示契约 `callerDisplayContractVerified=true`，生产 audit log 可观测，最近 30 天错误率低于 1%。该记录早于 explanation 入口 smoke 覆盖，Stage 5A 解释入口需使用更新后的 harness 复验；本地 `output/agent-smoke/` 报告属于运行产物，不纳入仓库。
...
- `business_patrol_diagnosis`：经营巡检。确定性接口为 `POST /api/agent/diagnosis/business-patrol`，并行聚合增长、成本指标、报价转化、续保追踪、赔案风险和客户流向六类诊断，输出经营异常优先级、受影响指标、推荐下钻能力、子诊断失败/超时 warning 和禁止解释汇总；不新增 SQL 生成器，不输出利润、盈利、亏损或承保利润结论。
...
- `forecast_operating_profit_scenario`：经营利润预测情景测算。确定性接口为 `POST /api/agent/forecast/profit-scenario`，基于调用方提供的终极变动成本率、终极固定成本率、签单保费和已赚率计划输出经营预测利润、分期释放和 1pct 成本率敏感性；禁止解释为财务报表利润、法定承保利润或审计利润。
