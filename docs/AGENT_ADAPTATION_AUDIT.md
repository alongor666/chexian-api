...
调用方展示证据已由 `scripts/verify-agent-production-smoke.mjs` 和 `tests/api/agent-production-smoke-harness.test.mjs` 锁定。底层确定性诊断接口和 explanation 入口必须继续返回 `warnings` 与 `forbiddenInterpretations`，调用方不得忽略这些字段。
