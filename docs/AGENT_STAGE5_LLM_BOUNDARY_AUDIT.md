...
Stage 5 可以进入设计评审，但不能直接打开 LLM 开关。当前生产 readiness 已满足 Stage 5 前置证据，`/api/agent/audit/readiness` 仍必须保持 `readyForLlm=false`，直到单独的 Stage 5 实现 PR 明确注册受保护入口、Zod schema、审计字段和红线测试。

本阶段只定义边界，不新增 LLM 调用，不新增聊天窗口，不生成 SQL，不自创指标。

## 当前代码事实

- `/api/agent/audit/*` 和 `/api/agent/diagnosis/*` 是 Agent 指标体系与确定性诊断的主路径。
- `server/src/agent/` 当前不调用 LLM，不导入 `getDefaultLlmProvider`，不注册 `agent/explain` 路由。
