...
def test_sichuan_postal_instance_isolates_to_sc() -> None:
    """四川邮政实例 filters 必须带 branch_code='SC'，policy_glob 必须排除 SX 分片。"""
    ...
    # glob 排他：排除 SX_ 大写 S 开头分片（[!S]*），不依赖裸 current/*.parquet
    assert "SX" not in inst.policy_glob or "[!S]" in inst.policy_glob, (
        f"四川邮政实例 policy_glob 未排除 SX 分片：{inst.policy_glob!r}"
    )
...
def test_shanxi_postal_instance_isolates_to_sx() -> None:
    """山西邮政实例 filters 必须带 branch_code='SX'，policy_glob 仅取 SX_ 分片。"""
    ...
    assert "SX_" in inst.policy_glob, (
        f"山西邮政实例 policy_glob 应仅取 SX_ 分片：{inst.policy_glob!r}"
    )
