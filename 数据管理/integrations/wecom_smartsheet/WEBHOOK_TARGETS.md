...
| `shanxi_taiyuan2_renweijun` | 太原二部机构级「业务台账」（现表原地升级·🔶调试态限任卫军数据） | `.env.local:WECOM_SMARTSHEET_WEBHOOK_TY2_RENWEIJUN` | `state/shanxi-taiyuan2-renweijun_synced_keys.json`（add）+ `state/shanxi-taiyuan2-renweijun_record_map.json`（update） | `instances/shanxi-taiyuan2-renweijun.yaml` | 双引擎：add 增量写 10 字段（2025-08-01 起签单：出单日期/保单号/车牌号/投保人/保费/险种大类/客户类别/上年保险公司/定价-签单/商NCD-签单）；update 引擎（`sync_ledger_update_fields.py`，daily.mjs 按 update_sync 块路由）刷新续保 4 字段（是否报价-续保/风险等级-续保/定价-续保/商NCD-续保 = 续保底册应续 × 报价域"签单日+30 天后"最新报价）；省份隔离 branch_code='SX'；🔶调试态 filters 限业务员=118046126任卫军，扩散全机构=显式放宽为 org_level_3 过滤 + --rebuild-state | 终态=每三级机构一表一 webhook（2026-07-18 定案，行权限用户实测可用；权限/视图表端配置）；业务员成员列待用户加列后启用（花名册 `rosters/sx-taiyuan2-userids.json` 工号→企微userid，缺失行留空仅管理员可见）；record_id 由 add 响应捕获（通道铁律：永不依赖机器人授权，prime-state 仅历史兜底）；投保人敏感字段 dry-run/日志脱敏；手机号码业务员自填；业务员文本列写入经 `field_transforms: chinese_only` 只留中文姓名（2026-07-23 用户要求，add/update 双引擎生效，存量行随 update 引擎 payload_hash 变化自动刷新）；调试交付生产后本表模板化 |
