...
## ⚠ 设计语义说明（Claude review #414 提示）

### 1. ClaimsAgg 件数 / 金额过滤不对称（**已知设计**）

修法 C 在 `duckdb-domain-loaders.ts:createClaimsAggFromDetail` 里：
- `claim_cases = COUNT(DISTINCT claim_no)` **不加过滤**（保持件数 cohort 与 xlsx 对齐）
- `reported_claims = SUM(CASE ...)` 在 SUM 内过滤掉 `liability_ratio=0` + `case_type ∈ {零结/注销/拒赔}`

**下游影响**：
- `avg_claim_amount = reported_claims / claim_cases` 分母含异常案件、分子不含 → 案均会系统性偏低
- `earned_loss_frequency = claim_cases / 满期暴露` 含无赔款的报案件 → 频度看起来偏高

**这是与 xlsx 对齐的必然设计**：xlsx "已报件数 4871" 实际包含异常案件（所以项目不过滤件数），但 xlsx "已报赔款" 排除异常案件（所以项目过滤金额）。这两个事实独立成立，**不是 bug**，但分析人员引用 `avg_claim_amount` / `earned_loss_frequency` 时需明确口径。

### 2. 对账脚本 vs 生产 ClaimsAgg 的 `settlement_time` 过滤差异

| | 对账脚本 (`project_loader.py`) | 生产 (`duckdb-domain-loaders.ts`) |
|---|---|---|
| settlement_time 过滤 | `IS NOT NULL AND CAST(... AS DATE) <= cutoff` | `IS NOT NULL`（无 cutoff） |
| 场景 | "截至 5/16 当时的赔款" | "截至最新的赔款" |
| 已结案选择 | 只取 5/16 前结案的 settled，5/16 后结案的算 reserve | 取全部 settled |

cutoff 后才登记结案时间的赔款：对账脚本算 reserve（估损值），生产算 settled（实付值），同一 cutoff 下会产生系统性差异。

**这是周报口径与生产口径的本质区别**：周报要"5/16 当时的快照"，生产要"现在最准的累计"。对账脚本的过滤是正确选择。
