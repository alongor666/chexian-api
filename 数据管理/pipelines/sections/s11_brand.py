#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""板块 11: 品牌车型维度 — JOIN dim/brand/latest.parquet 提取品牌+车型分类
...
def _write_brand_table(rpt, brand_data, brand_names):
    ...
    rpt.add("| 品牌_车型分类 | 保单数 | 签单premium | 满期premium | 赔款 | 赔付率 | 出险率 | 案均赔款† | 费用率 | 变动成本率 | 边际贡献额 | 系数 |")
    rpt.add("|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
...
def run(ctx, rpt, collected, silent=False):
    ...
    if not Path(BRAND_DIM).exists():
        msg = (f"> ⚠️ 品牌维度表不存在: {BRAND_DIM}\n"
               "> 运行 `node 数据管理/daily.mjs brand` 或 `python3 数据管理/pipelines/convert_brand_dim.py` 生成\n")
        if not silent:
            rpt.add("## 11. 品牌维度\n")
            rpt.add(msg)
    ...
    brand_names = [r["brand_key"] for r in brand_rows]
    brand_data = {r["brand_key"]: r for r in brand_rows}
    ...
    rpt.add("## 11. 品牌维度\n")
    rpt.add(f"### 11.0 品牌×车型分类汇总（保单≥{MIN_POLICIES}，共 {len(brand_names)} 个组合，按premium降序）\n")
