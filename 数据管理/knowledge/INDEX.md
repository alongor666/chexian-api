...
| 生成维度表 | generate_dim_tables.py | `cd 数据管理 && python3 数据管理/warehouse/dim/generate_dim_tables.py` |
...
| 输入 | 输出 |
|------|------|
| `每日数据_*.xlsx`（源数据） | `数据管理/warehouse/fact/policy/current/*.parquet` |
| `02_理赔明细_*.xlsx` | `数据管理/warehouse/fact/claims_detail/claims_*.parquet` |
| `04_报价清单*.xlsx` | `数据管理/warehouse/fact/quotes_conversion/latest.parquet` |
| `shard-config.json`（分片配置） | |

调用链: `daily.mjs` → `transform.py` / `quote_etl.py` / 各域转换器 → `sync-vps.mjs`

### 4.2 transform.py — Excel→Parquet 核心转换
...
### 4.3 quote_etl.py — 报价转化 ETL

**路径**: `数据管理/pipelines/quote_etl.py`
**状态**: 现役（daily.mjs quotes 子命令调用）

输入: `旧车商业险报价*.xlsx` + `数据管理/warehouse/dim/salesman/latest.parquet`
输出: `数据管理/warehouse/fact/quotes_conversion/latest.parquet`
...
### 4.6 merge_parquet.py — Parquet 合并
...
| 本地 | VPS |
|------|-----|
| `数据管理/warehouse/fact/policy/current/` | `server/data/current/` |
| `数据管理/warehouse/dim/salesman/` | `server/data/dim/salesman/` | <!-- governance-allow: index-doc-links 数据目录仅存在于数据机，目录级不在 gitignore 文件规则内 -->
| `数据管理/warehouse/dim/plan/` | `server/data/dim/plan/` | <!-- governance-allow: index-doc-links 同上 -->
| `数据管理/warehouse/fact/renewal_tracker/` | `server/data/fact/renewal_tracker/` | <!-- governance-allow: index-doc-links 同上 -->
...
**数据连接**: 内存 DuckDB 直读 `数据管理/warehouse/fact/policy/current/*.parquet`（绕过 PolicyFact 视图）
...
**数据连接**: 内存 DuckDB 直读 `数据管理/warehouse/fact/policy/current/*.parquet`（`经代名` 字段未进 PolicyFact）
...
```bash
cd 数据管理 && python3 数据管理/warehouse/dim/generate_dim_tables.py
```
...
| 输出 | 说明 |
|------|------|
| `数据管理/warehouse/dim/salesman/latest.parquet` | 业务员主数据（296 人，含编号/姓名/团队/机构/岗位/状态/入离职） |
| `数据管理/warehouse/dim/plan/latest.parquet` | 多年多层级计划（2025 业务员 + 2026 业务员 + 2026 机构 = 484 行） |
| `数据管理/warehouse/dim/dim_summary.json` | 统计摘要 |
...
| `数据管理/archive/legacy-scripts/daily.sh` | bash 版 ETL | `daily.mjs` |
| `数据管理/archive/legacy-scripts/sync-data.sh` | bash 版同步 | `scripts/sync-vps.mjs` |
| 旧 CLI 工具目录（data_tools/ 等） | 已删除，INDEX.md v1.0 遗留引用 | `数据管理/pipelines/` 下实际脚本 |
...
PolicyFact (VIEW) → PolicyFactRealtime (TABLE，物化)
    ...
    │
SalesmanDim (TABLE ← server/data/dim/salesman/latest.parquet)
PlanFact (TABLE ← server/data/dim/plan/latest.parquet)
SalesmanTeamMapping (TABLE ← SalesmanDim LEFT JOIN PlanFact)
SalesmanPlanFact (VIEW ← PlanFact LEFT JOIN SalesmanDim，多年计划)
QuoteConversion (VIEW ← server/data/fact/quotes_conversion/*.parquet，透传)
RenewalTrackerFact (VIEW ← server/data/fact/renewal_tracker/latest.parquet)

── 8 域分域架构新增（2026-04） ──
ClaimsDetail (VIEW ← server/data/fact/claims_detail/latest.parquet，赔案级明细)
ClaimsAgg (TABLE ← 从 ClaimsDetail 动态聚合，唯一来源)
CrossSellFact (TABLE ← server/data/fact/cross_sell/latest.parquet，独立交叉销售清单)
RepairDim (TABLE ← server/data/dim/repair/latest.parquet，维修资源维度)
BrandDim (TABLE ← server/data/dim/brand/latest.parquet，品牌车型维度)
CustomerFlow (VIEW ← server/data/fact/customer_flow/latest.parquet，客户来源去向)
```
