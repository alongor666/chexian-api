...
#   ./scripts/sync-vps.mjs                     # 自动同步 current/ 下最新文件
#   ./scripts/sync-vps.mjs 文件路径            # 指定同步某个文件
#   ./scripts/sync-vps.mjs 文件路径 --no-restart  # 同步但不重启（批量同步中间文件用）
#   ./scripts/sync-vps.mjs 文件路径 --clean-vps   # 上传前清理 VPS 的 current/ 目录（将旧文件移入 archive）
# ============================================================
...
if [ -z "$FILE" ] || [ ! -f "$FILE" ]; then
    ...
    echo "用法: ./scripts/sync-vps.mjs [文件路径] [--no-restart]"
    echo "      ./scripts/sync-vps.mjs --export    # 预聚合模式（推荐）"
    exit 1
fi
...
if [ "$CLEAN_VPS" = true ]; then
    ...
    # 自动清理 VPS 上超过 30 天的历史归档，防止磁盘随时间被占满
    echo -e "${YELLOW}  清理 VPS 上超过 30 天的过期归档...${NC}"
    ssh "$VPS_HOST" "find ${VPS_DATA}/archive -type d -name 'backup_*' -mtime +30 -exec rm -rf {} + 2>/dev/null || true"
fi
