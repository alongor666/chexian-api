...
import { EARLY_BATCH, LATE_BATCH, batchAllCodes } from '../数据管理/lib/release-batches.mjs';

const TODAY = '2026-07-04';
const STALE_MTIME = '2026-07-01T00:00:00.000Z'; // 北京 2026-07-01 ≠ TODAY
...
describe('planBackfillFiles（契约外补导文件识别 — 2026-07-05 上游补导 02 报价 0624-0703 实证）', () => {
  ...
  it('🔴 双批：allowedCodes=[01,05] 时不把上游残留的 02 补导误分发进早批', () => {
    const inbox = [
      'shanxi_20260624_02_报价清单_商业险.xlsx', // 昨日残留报价，属晚批 → 早批不应分发
      'shanxi_20240101-20250531_01_签单清单_定稿.xlsx', // 补导签单 → 早批应分发
    ];
    expect(planBackfillFiles(inbox, CURRENT, batchAllCodes(EARLY_BATCH))).toEqual([
      'shanxi_20240101-20250531_01_签单清单_定稿.xlsx',
    ]);
  });
});

describe('🔴 双批 code 子集校验（早批只看 01/05，晚批只看 02/03/04；互不阻塞）', () => {
  it('早批：02/03 停在往日也不拦早批就绪（只看 01/05）', () => {
    const reports = [
      report('01'), report('05'),
      report('02', { mtime: STALE_MTIME }), report('03', { mtime: STALE_MTIME }),
    ];
    ...
    expect(r.ready).toBe(true);
    expect(r.reports.map((x: { code: string }) => x.code).sort()).toEqual(['01', '05']);
  });

  it('早批：01 停在往日 → 早批未就绪', () => {
    const reports = [report('01', { mtime: STALE_MTIME }), report('05')];
    const r = evaluateRemoteManifest(manifestOf(reports), {
      todayBeijing: TODAY,
      requiredCodes: batchAllCodes(EARLY_BATCH),
      optionalCodes: EARLY_BATCH.optionalCodes,
  ...
  it('晚批：01/05 缺席/停在往日也不拦晚批就绪（只看 02/03；04 可选）', () => {
    const reports = [
      report('02'), report('03'),
      report('04', { mtime: STALE_MTIME }), // 04 周一至周六停在上周日 → 可选，warn 不拦
      report('01', { mtime: STALE_MTIME }),
    ];
    const r = evaluateRemoteManifest(manifestOf(reports), {
      todayBeijing: TODAY,
      requiredCodes: batchAllCodes(LATE_BATCH),
      optionalCodes: LATE_BATCH.optionalCodes,
    });
    expect(r.ready).toBe(true);
    // 04 停在往日 → warn 且不进 reports（保留旧维表）；硬闸 02/03 进
    expect(r.reports.map((x: { code: string }) => x.code).sort()).toEqual(['02', '03']);
    expect(r.issues.some((i: { level: string }) => i.level === 'error')).toBe(false);
  });

  it('晚批：03 维修停在往日 → 晚批未就绪', () => {
    const reports = [report('02'), report('03', { mtime: STALE_MTIME }), report('04')];
    const r = evaluateRemoteManifest(manifestOf(reports), {
      todayBeijing: TODAY,
      requiredCodes: batchAllCodes(LATE_BATCH),
      optionalCodes: LATE_BATCH.optionalCodes,
  ...
  it('evaluateManifestReports 早批只分发 01/05（不校验/不分发 02/03）', () => {
    const reports = [report('01'), report('05'), report('02'), report('03')];
    const r = evaluateManifestReports(manifestOf(reports), {
      todayBeijing: TODAY,
      statByName: statsFor(reports),
      requiredCodes: batchAllCodes(EARLY_BATCH),
      optionalCodes: EARLY_BATCH.optionalCodes,
    });
    expect(r.ok).toBe(true);
  ...
  it('默认（不传 requiredCodes）仍是全 5 张 code — 拆批前逐字节兼容', () => {
    const r = evaluateRemoteManifest(manifestOf(fullBatch()), { todayBeijing: TODAY });
    expect(r.ready).toBe(true);
    expect(r.reports.length).toBe(REQUIRED_REPORT_CODES.length);
  });
});
