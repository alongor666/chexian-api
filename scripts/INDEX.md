...
| `gen-reports-manifest.mjs` | **静态报告 manifest 生成**：扫描 `public/reports/<slug>/`（含 B346 机构级子目录 `orgs/<branch>/<org>/`，为每个机构目录生成带 `scope` 的 manifest），把真实存在的报告日期写入 `manifest.json`，供首页卡片按用户可见范围解析「应打开哪一期 + 是否落后于 etlDate」。由 `sync-vps.mjs` / `sync-and-reload.mjs` 同步前自动调用 | `bun run reports:manifest` |
...
- 新增 `check-hotfile-contracts.mjs`：当 `server/src/routes/query.ts`、`src/shared/api/client.ts`、`src/shared/api/client-core.ts`（传输内核）或任一命名空间子客户端 `src/shared/api/*-api.ts`（Phase 2 神类拆分后 81 个迁出业务方法的新归处，清单从文件系统 glob 派生、非硬编码；守恒计数权威源 `api-wire-conservation.mjs`）进入暂存区时，要求同步修改契约测试（`client-contracts` / `client-core-transport` / `*route-contract`）。防"改子客户端不触发门禁→契约静默漂移"的掏空陷阱。
