...
| 校验 ApiClient 拆分守恒（原99 = 保留25 + Σ命名空间74，零搬丢/改线） | `api-wire-conservation.mjs` | `node scripts/api-wire-conservation.mjs`（已入 `bun run governance`） |
...
| `api-wire-conservation.mjs` | **ApiClient 拆分守恒恒等式**：pre-#536 单体 99 业务方法 == 保留基类 25 + Σ命名空间 74；金 master golden 覆盖齐全（≥ 和）；路由集 LOST=∅（需 git 历史，缺则跳过）。冻结基线 `tests/api/__golden__/pre536-business-methods.json`（git 0e592603）。`--reseed` 重生基线，`--quiet-pass` 供 governance | `node scripts/api-wire-conservation.mjs`（已入 governance #25） |
...
- 新增 `check-hotfile-contracts.mjs`：当 `server/src/routes/query.ts`、`src/shared/api/client.ts`、`src/shared/api/client-core.ts`（传输内核）或任一命名空间子客户端 `src/shared/api/*-api.ts`（Phase 2 神类拆分后 74 个迁出业务方法的新归处，清单从文件系统 glob 派生、非硬编码；守恒计数权威源 `api-wire-conservation.mjs`）进入暂存区时，要求同步修改契约测试（`client-contracts` / `client-core-transport` / `*route-contract`）。防"改子客户端不触发门禁→契约静默漂移"的掏空陷阱。
