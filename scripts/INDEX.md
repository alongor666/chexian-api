...
| `check-data-readiness.mjs` | **数据就绪校验**（5 项数据状态）：Parquet 重叠 / Claims 去重 / 知识库规模 / 单文件不混省（派生省==列省 · 多省 Phase 4）/ 同步漂移；由 release:daily（`sync-and-reload.mjs` Stage 1.7）在 ETL 后、发布前执行，**不在**代码门禁跑 | `node scripts/check-data-readiness.mjs` |
