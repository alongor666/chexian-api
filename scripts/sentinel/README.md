...
每日 ETL 后对核心业务指标做「当前 vs 历史」对比：**统计层确定性判定是否告警 + 确定性规则归因**，异常才在 GitHub 追踪 issue 告警，无异常静默。业务深度归因走本地 skill `/chexian-sentinel-attribution`（不依赖外部 LLM API key，复用 Claude Code 主体能力）。

## 组成

| 文件 | 作用 |
|------|------|
| `etl-anomaly-sentinel.mjs` | 主编排：幂等门 → 取数 → 成熟度过滤 → 统计判定 → 规则归因 → 产出。**不碰 GitHub** |
| `sentinel.config.json` | 声明式配置：监控指标 + 各自阈值/方向/环比同比门、成熟度排除窗口、silence 设置 |
...
| `lib/llm-judge.mjs` | 确定性规则归因（保留文件名为兼容 import；2026-06-14 起不再调外部 LLM API） |
...
→ 统计判定（排除未成熟近期，IBNR 防线）→ 规则归因 → verdict.json + summary.md
```
...
- **成熟度过滤（IBNR）**：满期赔付率近期受赔款报告滞后影响系统性偏低、随时间向上发展（参考 `diagnose-loss-development`）。统计判定**排除最近 N 个完整期**，只对已成熟期判异常——这是确定性处理，可复现。
- **逐期而非累计**：比率 Z-score 必须用 per-period 值（comprehensive lossTrend 即逐期），禁止用累计序列（强自相关会压低标准差、频繁误触）。
- **统计判定 + 规则归因**：是否告警 100% 由统计层决定（可复现）；归因列由确定性规则给出 severity + 一句话原因（如「环比 19.4% 超阈值 8%」）。业务深度归因（IBNR / 渠道结构变化 / 数据成熟度伪信号识别等）由本地 skill `/chexian-sentinel-attribution` 按需触发，复用 Claude Code 主体能力与项目上下文（指标字典 / 业务规则字典 / 历史诊断报告），不依赖 GH Actions 外部 LLM API key。
...
> 2026-06-14 起不再消费 `ANTHROPIC_API_KEY` / `ZHIPU_API_KEY` — LLM 归因已剥离至本地 skill。历史已配的 secret 可一并清理。
