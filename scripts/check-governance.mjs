...
function checkDualLockConsistency() {
  info('检查双锁一致性（server/package.json 声明范围 vs server/package-lock.json 锁定版本）...');

  const pkgJsonPath = path.join(ROOT_DIR, 'server/package.json');
  const lockPath = path.join(ROOT_DIR, 'server/package-lock.json');
...
/**
 * 通用锚点镜像文本提取 + 对账 helper（供能力矩阵 / 省份映射等前后端镜像检查复用）
 */
...
function compareAnchoredMirror(label, frontPath, backPath, beginMarker, endMarker) {
 ...
 */
function checkBranchMappingMirror() {
  info('检查省份映射两端一致（BRANCH_LABELS/BRANCH_NAMES + SX_ORGANIZATIONS/BRANCH_ORGANIZATIONS 前后端镜像）...');

  const labelsFrontPath = path.join(ROOT_DIR, 'src/shared/utils/branchDisplay.ts');
  const labelsBackPath = path.join(ROOT_DIR, 'server/src/config/branch-names.ts');
  const orgFrontPath = path.join(ROOT_DIR, 'src/shared/config/organizations.ts');
  const orgBackPath = path.join(ROOT_DIR, 'server/src/services/permission.ts');

  let ok = true;
  ok = compareAnchoredMirror(
    '省份中文名映射（BRANCH_LABELS/BRANCH_NAMES）',
    labelsFrontPath, labelsBackPath,
    'BRANCH-NAME-MIRROR-BEGIN', 'BRANCH-NAME-MIRROR-END',
  ) && ok;
  ok = compareAnchoredMirror(
    '山西经营单元列表（SX_ORGANIZATIONS）',
    orgFrontPath, orgBackPath,
    'SX-ORG-MIRROR-BEGIN', 'SX-ORG-MIRROR-END',
  ) && ok;
  ok = compareAnchoredMirror(
    '分公司→机构列表（BRANCH_ORGANIZATIONS）',
    orgFrontPath, orgBackPath,
    'BRANCH-ORG-MIRROR-BEGIN', 'BRANCH-ORG-MIRROR-END',
  ) && ok;
...
const CODE_GOVERNANCE_CHECKS = [
  ...
  { name: '双锁一致性', fn: checkDualLockConsistency },
  { name: '省份映射前后端镜像', fn: checkBranchMappingMirror },
