...
// ============================================================
// buildPolicyCurrentTasks：布局感知任务构建（B3 核心）
// ============================================================
describe('buildPolicyCurrentTasks — 布局感知（扁平 / 子目录 / 非基准省排除）', () => {
  ...
  it('子目录独占·仅基准省（current/SC/，基准=SC）→ 每省独立任务 current/SC/ → data/current/SC', () => {
  ...
  it('子目录含非基准省（current/SC/ + current/SX/，基准=SC）→ 仅生成 SC 任务（SX 防御性排除）', () => {
    const dir = makeCurrent({ subdirs: { SC: ['a.parquet'], SX: ['x.parquet'] } });
    const tasks = buildPolicyCurrentTasks(dir, '/remote/current', 'SC');
    // 防御性排除非基准省：不静默把 SX 推生产（GATED 预检会大声 fail-closed，见 helper 测试）
    expect(tasks).toHaveLength(1);
    expect(tasks[0].label).toBe('policy/current/SC');
    expect(tasks.some((t) => t.label.includes('SX'))).toBe(false);
  });

  it('基准省=SX 时 → 仅生成 SX 子目录任务（基准省由参数驱动，非硬编码 SC）', () => {
    const dir = makeCurrent({ subdirs: { SX: ['x.parquet'] } });
...
describe('buildStandardSyncTasks — policy 任务布局感知 + 其余 13 域不变', () => {
  ...
  it('子目录 localCurrentDir 注入（仅基准省 SC）→ policy/current/SC 任务', () => {
...
describe('真实 rsync — 扁平 SC 字节安全 + 分省子目录隔离 + --delete 限子目录', () => {
  ...
  it.skipIf(!rsyncAvailable)('分省隔离：current/SC/ → data/current/SC/，--delete 不触碰 data/current/SX/（跨省隔离 + 作用域）', () => {
    ...
    // buildPolicyCurrentTasks（基准=SC）→ 仅 SC 任务，目标 data/current/SC
