...
/**
 * 时间窗约束：只看「已完整结束的月份」。
 *   - insurance_start_date 可远超 current_date（预签保单），不过滤会把未来月当成断崖触发。
 *   - 用 `< date_trunc('month', current_date)` 把当前不完整月也剔除，stats 的 excludeRecent
 *     再把上一个月（可能仍有迟到 ETL）排掉，得到稳定基线。
 *   - current_date 由 DuckDB 取系统时间；如需可复现测试，在调用层注入固定日期参数（P3）。
 */
const COMPLETED_MONTH_FILTER = (col) =>
  `${col} IS NOT NULL AND ${col} < date_trunc('month', current_date)`;

/** SQL 模板表 — key = config.metric.source，value = (globPaths) => sql */
export const SQL_TEMPLATES = {
  /** 月签单保费：policy_dedup 后按月聚合 SUM(premium)。口径 = cost-ratios.ts B252 */
  'policy_dedup.monthly_premium': ({ policyGlob }) => `
    WITH policy_dedup AS (
      ...
      WHERE ${COMPLETED_MONTH_FILTER('insurance_start_date')}
  ...
  'policy_dedup.monthly_policy_count': ({ policyGlob }) => `
    WITH policy_dedup AS (
      SELECT
        policy_no,
        CAST(insurance_start_date AS DATE) AS start_date,
    ...
    ORDER BY time_period
  `,
  /** 月出险报告金额：按 accident_time 月分组，settled + reserve（项目标准口径） */
  'claims_detail.monthly_claim_amount': ({ claimsGlob }) => `
    SELECT
      strftime(accident_time, '%Y-%m') AS time_period,
      ROUND(SUM(COALESCE(settled_amount, 0) + COALESCE(reserve_amount, 0)), 2) AS value
    FROM read_parquet('${claimsGlob}')
    WHERE ${COMPLETED_MONTH_FILTER('accident_time')}
    GROUP BY time_period
    ORDER BY time_period
  `,
  /** 月出险报案件数：COUNT DISTINCT claim_no by accident_time month */
  'claims_detail.monthly_claim_count': ({ claimsGlob }) => `
    SELECT
      strftime(accident_time, '%Y-%m') AS time_period,
