...
export function sessionPrompt({ task, domains }) {
  ...
  return [
    ...
    `4. 🛡 闸-2（${String(task.priority) <= 'P1' ? '⚠️ 本任务 P0/P1 复杂级·强制不可跳' : '按复杂度'}）：codex 审 diff——**code review 单源=codex CLI**（§2 降级分层：codex skill → CLI \`codex exec --sandbox read-only - < prompt\`；CLI 全不可用→标 unavailable 向用户报缺口请授权，不回退 evidence-verifier/CI）。P0/P1 全修 + 复审通过。**不起 code-reviewer/evidence-verifier 子代理，不计 CI auto-review 为闸源**；correctness 由第 3 步确定性闸承担。`,
    `5. 收尾：backlog status 流转 + pr-evolution 三问复盘(needs_automation 紧跟 expires) + loop-quality-ledger 一行 → bundle 进代码提交 → PR。`,
    `6. 🚦 合并门 + 自动合并：闸-2（codex 单源）P0/P1 全清 + CI 双绿 + **非部署链** → \`bun run loop:dispatch --merge-gate\` 确认 slot holder 后 \`gh pr merge --auto --squash\` **自动合并**（P0/P1 单任务无需人工交接；enable --auto 后禁再 push）。非 slot 则等前序落地 main → git fetch+merge 重新转绿再 enable。🔴 **部署链 PR**（deploy.yml/vps-wrapper/sync-vps/ecosystem）禁 auto-merge，人工选窗口合并并盯 CI 前 5 分钟。`,
    ``,
