...
export async function analyzeOrgTrendWithZhipu(
  ...
  const dataStr = rows
    .map(r => `${r.date} 车险${r.auto_count}件 驾意${r.driver_count}件 推介率${r.rate}% 驾意件均${r.avg_premium}元`)
    .join('\n');

  const systemPrompt = `你是车险业务分析专家。请根据机构最近每日推介率和驾意件均数据，输出两段式趋势分析（总字数220字以内）。
硬性格式要求：
1) 第一段只讲“推介率”；
2) 第二段只讲“驾意件均”。
