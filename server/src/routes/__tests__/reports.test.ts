...
function makeReq(user?: unknown, query?: unknown) {
...
describe('resolvePortalScope: 全国超管消费 UI 切省信号 targetBranch（白名单校验·2026-07-09-claude-9692f9）', () => {
  // 全国超管：默认省四川、可切 SC/SX（镜像 preset xuechenglong）
  const nat = (query?: unknown) =>
    makeReq({ role: UserRole.BRANCH_ADMIN, branchCode: 'SC', visibleBranches: ['SC', 'SX'] }, query);

  it('切到授权省 SX → scope.branch=SX；fullEntitlement=false（切他省不享根目录 legacy 回退，防四川数据泄漏）', () => {
    expect(resolvePortalScope(nat({ targetBranch: 'SX' }))).toEqual({
      kind: 'branch',
      branch: 'SX',
  ...
  it('切回默认/部署省 SC → scope.branch=SC（fullEntitlement=false，但 SC===部署省仍可回退根目录）', () => {
    expect(resolvePortalScope(nat({ targetBranch: 'SC' }))).toEqual({
      kind: 'branch',
      branch: 'SC',
      fullEntitlement: false,
    });
  ...
  it('无 targetBranch → 回落默认省 SC + fullEntitlement（未切省零行为变化）', () => {
    expect(resolvePortalScope(nat())).toEqual({ kind: 'branch', branch: 'SC', fullEntitlement: true });
    expect(resolvePortalScope(nat({}))).toEqual({ kind: 'branch', branch: 'SC', fullEntitlement: true });
  });

  it("targetBranch='ALL'（全国合并视图）门户不适用 → 回落默认省 SC（不落进坏路径）", () => {
    expect(resolvePortalScope(nat({ targetBranch: 'ALL' }))).toEqual({
      kind: 'branch',
      branch: 'SC',
      fullEntitlement: true,
    });
  ...
  it('越权 targetBranch（未授权省 / 非法格式 / 空 / 非字符串）→ 一律忽略，回落默认省（绝不越权他省）', () => {
    for (const bad of ['GD', 'gd', 'SXX', 'S1', '', 123, ['SX'], null, { x: 1 }]) {
      expect(resolvePortalScope(nat({ targetBranch: bad }))).toEqual({
        kind: 'branch',
        branch: 'SC',
        fullEntitlement: true,
      });
  ...
  it('visibleBranches 全脏值被过滤空 → 绝不采用 targetBranch，回落默认省', () => {
    expect(
      resolvePortalScope(
        makeReq(
          { role: UserRole.BRANCH_ADMIN, branchCode: 'SC', visibleBranches: ['sc', 'x'] },
          { targetBranch: 'SX' }
        )
      )
    ).toEqual({ kind: 'branch', branch: 'SC', fullEntitlement: true });
  ...
  it('单省 branch_admin（无 visibleBranches）带 targetBranch → 忽略（无切省口子，scope 固定本省）', () => {
    expect(
      resolvePortalScope(
        makeReq({ role: UserRole.BRANCH_ADMIN, branchCode: 'SX' }, { targetBranch: 'SC' })
      )
    ).toEqual({ kind: 'branch', branch: 'SX', fullEntitlement: false });
  });

  it('org_user 带 targetBranch → 完全忽略（scope 由服务端派生，无越权他机构/他省口子）', () => {
    expect(
      resolvePortalScope(
        makeReq(
          { role: UserRole.ORG_USER, organization: '乐山', branchCode: 'SC' },
          { targetBranch: 'SX' }
        )
      )
    ).toEqual({ kind: 'org', branch: 'SC', org: '乐山' });
