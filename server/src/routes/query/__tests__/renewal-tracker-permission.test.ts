...
/**
 * resolveBranchRlsCode 真实逻辑内联（供 mock shared.js 用）。
 * 复刻 shared.ts 的 gate a（permissionFilter 含 branch_code='XX'）；gate b（视图实测含
 * branch_code 列）在无原生 DuckDB 的单测中视为恒成立——这正是 RLS-on + 派生域已补列后的
 * 生产形态，故单测以「列存在」为前提断言 typed 路由是否正确下推 branch_code。
 */
...
//    保留 buildOrgScopedPermissionWhere / resolveBranchRlsCode 真实逻辑 ─────────────────
vi.mock('../shared.js', () => ({
  asyncHandler: (fn: any) => fn,
  AppError: class AppError extends Error {
    statusCode: number;
...
describe('§ B: renewal-tracker 路由处理器：generateRenewalTrackerQuery 调用参数验证', () => {
  ...
  // RT-R-03：多分公司 org_user（org_level_3 AND branch_code）→ 两段都下推（org_level_3 + branch_code）
  it('RT-R-03: 多分公司 org_user（org_level_3=天府 AND branch_code=SC）→ extraConditions 同时含 org_level_3 与 branch_code=SC（双段隔离）', async () => {
    await handler(makeRouteReq("org_level_3 = '天府' AND branch_code = 'SC'"), makeRouteRes());
    expect(mockGenerate).toHaveBeenCalledOnce();
    const [{ extraConditions }] = mockGenerate.mock.calls[0];
    expect(extraConditions.some((c: string) => c.includes("org_level_3 = '天府'"))).toBe(true);
    expect(extraConditions.some((c: string) => c.includes("branch_code = 'SC'"))).toBe(true);
    expect(extraConditions.every((c: string) => !c.includes('is_telemarketing'))).toBe(true);
  });
  ...
  // RT-R-05：山西 branch_admin（仅 branch_code，无 org_level_3）→ 必须下推 branch_code（堵跨省串读）
  // 这是本次修复的核心回归：旧实现 buildOrgScoped 对其返回 '1=1' → 不注入 → 看全省（串读）。
  it('RT-R-05: 山西 branch_admin（仅 branch_code=SX，无 org_level_3）→ extraConditions 含 branch_code=SX（堵跨省串读）', async () => {
    await handler(makeRouteReq("branch_code = 'SX'"), makeRouteRes());
    expect(mockGenerate).toHaveBeenCalledOnce();
    const [{ extraConditions }] = mockGenerate.mock.calls[0];
    expect(extraConditions.some((c: string) => c.includes("branch_code = 'SX'"))).toBe(true);
    // branch_admin 无 org_level_3 段
    expect(extraConditions.every((c: string) => !c.includes('org_level_3'))).toBe(true);
    expect(extraConditions.every((c: string) => !c.includes('is_telemarketing'))).toBe(true);
  });

  // RT-R-06：meta（universe 统计）查询按 branchCode 下推，防元数据串读
  it('RT-R-06: meta 查询按 branchCode 下推 — branch_admin→收到 SX；单租户(1=1)→undefined（字节安全）', async () => {
    await handler(makeRouteReq("branch_code = 'SX'"), makeRouteRes());
    expect(mockGenerateMeta).toHaveBeenCalledWith('SX');

    vi.clearAllMocks();
    await handler(makeRouteReq('1=1'), makeRouteRes());
    expect(mockGenerateMeta).toHaveBeenCalledWith(undefined);
  });
});
