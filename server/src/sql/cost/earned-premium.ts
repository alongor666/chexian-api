 ...
 *    |-----------|-----------------------------------|--------------------------------------|
 *    | 终保日     | 起保日 + 1 YEAR − 1 DAY（闰年感知）| 起保日 + INTERVAL 1 YEAR（闰年感知） |
 ...
 *
 *    闰年口径（B304 · 2026-07-10 用户拍板统一）：终保日 = 起保日 + 1 YEAR − 1 DAY
 *    （闰年感知，与分母 policy_term、V3 矩阵 sql-builder.ts、业务规则字典权威口径一致）。
 *    旧约定固定 +364 天会把闰年跨越保单（policy_term=366）第 366 个在保日永久截掉
 *    （SC 账本终身欠计 84.41 万元）；当前至 2027-02 生产滚动窗口内无闰年保单，切换零扰动。
 *    量化实证见 `开发文档/审计/B304_已赚保费双口径闰年评估与字段迁移方案_2026-07-10.md`。
 */
...
WITH policy_earned AS (
  SELECT
    ...
    -- 终保日 = 起保日 + 1 年 − 1 天（闰年感知，B304 2026-07-10 拍板统一）
    CAST(insurance_start_date AS DATE) + INTERVAL 1 YEAR - INTERVAL 1 DAY AS end_date,
    ...
    GREATEST(
      ...
      DATEDIFF('day',
        ...
        LEAST(CAST(insurance_start_date AS DATE) + INTERVAL 1 YEAR - INTERVAL 1 DAY, DATE '${cutoffDate}')
      ) + 1
    ) AS days_in_window
  FROM PolicyFact
  WHERE ${whereClause}
    ...
    AND CAST(insurance_start_date AS DATE) <= DATE '${cutoffDate}'
    AND (CAST(insurance_start_date AS DATE) + INTERVAL 1 YEAR - INTERVAL 1 DAY) >= DATE '${windowStart}'
)
SELECT
