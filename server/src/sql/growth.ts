...
// ========== 【性能优化】V2版本 - 使用预聚合表 PeriodAggregated ==========

/**
 * 生成同比增长率查询SQL V2（使用预聚合表，性能提升15-30倍）
 *
 * 优化点：
 * - 使用 PeriodAggregated 预聚合表
 * - 使用 LAG 窗口函数替代 FULL OUTER JOIN
 * - 从几十万行扫描减少到几千行扫描
 *
 * @param config - 增长率配置
 * @returns SQL查询字符串
 */
export function generateYoYGrowthQueryV2(config: GrowthConfig): string {
  const { groupBy = [], whereClause = '1=1' } = config;
  const groupByClause = groupBy.length > 0 ? `, ${groupBy.join(', ')}` : '';
  const partitionByClause = groupBy.length > 0 ? `PARTITION BY ${groupBy.join(', ')}, policy_month` : 'PARTITION BY policy_month';
...
SELECT
  policy_ym AS time_period,
  ${groupBy.length > 0 ? groupBy.join(', ') + ',' : ''}
  period_premium AS current_value,
  COALESCE(prev_premium, 0) AS previous_value,
  CASE
    WHEN COALESCE(prev_premium, 0) = 0 THEN NULL
    ELSE (period_premium - prev_premium) / prev_premium
  END AS growth_rate
FROM ranked
WHERE prev_premium IS NOT NULL
ORDER BY time_period ${groupByClause}
  `.trim();
}

/**
 * 生成环比增长率查询SQL V2（使用预聚合表）
 *
 * @param config - 增长率配置
 * @returns SQL查询字符串
 */
export function generateMoMGrowthQueryV2(config: GrowthConfig): string {
  const { groupBy = [], whereClause = '1=1' } = config;
  const groupByClause = groupBy.length > 0 ? `, ${groupBy.join(', ')}` : '';
  const partitionByClause = groupBy.length > 0 ? `PARTITION BY ${groupBy.join(', ')}` : '';
...
SELECT
  ...
  CASE
    WHEN COALESCE(previous_value, 0) = 0 THEN NULL
    ELSE (current_value - COALESCE(previous_value, 0)) / previous_value
  END AS growth_rate
FROM with_lag
ORDER BY time_period ${groupByClause}
  `.trim();
...
/**
 * 生成年累计增长率查询SQL V2（使用预聚合表）
 *
 * @param config - 增长率配置
 * @returns SQL查询字符串
 */
export function generateYTDGrowthQueryV2(config: GrowthConfig): string {
  const { groupBy = [], whereClause = '1=1', referenceYear } = config;
  // DC-002: 使用传入的参考年份，若未提供则回退到当前年份
  const currentYear = referenceYear ?? new Date().getFullYear();
  const groupByClause = groupBy.length > 0 ? `, ${groupBy.join(', ')}` : '';
  const partitionByClause = groupBy.length > 0 ? `PARTITION BY policy_year, ${groupBy.join(', ')}` : 'PARTITION BY policy_year';
...
previous_ytd AS (
  SELECT
    policy_year + 1 AS aligned_year,
    policy_month,
    ${groupBy.length > 0 ? groupBy.join(', ') + ',' : ''}
    cumulative_value AS previous_cumulative
  FROM cumulative_data
  WHERE policy_year = ${currentYear - 1}
)
SELECT
  COALESCE(c.time_period, CONCAT(CAST(p.aligned_year AS VARCHAR), '-', LPAD(CAST(p.policy_month AS VARCHAR), 2, '0'))) AS time_period,
  ${groupBy.length > 0 ? groupBy.map(g => `COALESCE(c.${g}, p.${g}) AS ${g}`).join(', ') + ',' : ''}
  COALESCE(c.cumulative_value, 0) AS current_value,
  COALESCE(p.previous_cumulative, 0) AS previous_value,
  CASE
    WHEN COALESCE(p.previous_cumulative, 0) = 0 THEN NULL
    ELSE (COALESCE(c.cumulative_value, 0) - p.previous_cumulative) / p.previous_cumulative
  END AS growth_rate
FROM current_ytd c
FULL OUTER JOIN previous_ytd p ON c.policy_month = p.policy_month
  ${groupBy.length > 0 ? `AND ${groupBy.map(g => `c.${g} = p.${g}`).join(' AND ')}` : ''}
ORDER BY time_period ${groupByClause}
  `.trim();
}
 ...
 * @returns SQL查询字符串
 */
export function generateDualMetricComparisonQueryV2(
  config: DualMetricComparisonConfig
): string {
  const {
    currentStartDate,
  ...
  // 将日期转换为 YYYY-MM 格式
  const currentStartYM = currentStartDate.substring(0, 7);
  const currentEndYM = currentEndDate.substring(0, 7);
  const previousStartYM = previousStartDate.substring(0, 7);
  const previousEndYM = previousEndDate.substring(0, 7);

  return `
WITH current_data AS (
  SELECT
    ${groupBySelect}
    SUM(period_premium) AS current_premium,
    SUM(period_count) AS current_count
  FROM PeriodAggregated
  WHERE ${whereClause}
    AND policy_ym >= '${currentStartYM}'
    AND policy_ym <= '${currentEndYM}'
  GROUP BY ${groupByClause}
),
previous_data AS (
  SELECT
    ${groupBySelect}
    SUM(period_premium) AS previous_premium,
    SUM(period_count) AS previous_count
  FROM PeriodAggregated
  WHERE ${whereClause}
    AND policy_ym >= '${previousStartYM}'
    AND policy_ym <= '${previousEndYM}'
  GROUP BY ${groupByClause}
)
SELECT
  ${groupBy.length > 0
