...
// ========== 【性能优化】V2版本 - 使用预聚合表 EarnedPremiumMonthly ==========
 ...
 * @returns SQL查询字符串
 */
export function generateNewEarnedPremiumSummaryQueryV2(config: NewEarnedPremiumConfig = {}): string {
  // 注：whereClause 暂未使用，预聚合表已包含所有数据，未来可扩展支持条件筛选
  const { whereClause: _whereClause = '1=1' } = config;
...
WITH monthly_earned AS (
  SELECT
    stat_month,
    YEAR(CAST(policy_month || '-01' AS DATE)) AS policy_year,
    SUM(total_premium) AS total_premium,
    SUM(earned_premium) AS earned_premium
  FROM EarnedPremiumMonthly
  WHERE stat_month BETWEEN '2026-01' AND '2026-12'
  GROUP BY stat_month, YEAR(CAST(policy_month || '-01' AS DATE))
),
summary AS (
  SELECT
    stat_month,
    -- 滚动12个月保费（简化：使用当月所有保单的保费）
    SUM(total_premium) AS rolling_12m_premium,
    -- 2025年保单已赚保费
    SUM(CASE WHEN policy_year = 2025 THEN earned_premium ELSE 0 END) AS earned_from_2025,
    -- 2026年保单已赚保费
    SUM(CASE WHEN policy_year = 2026 THEN earned_premium ELSE 0 END) AS earned_from_2026
  FROM monthly_earned
  GROUP BY stat_month
)
SELECT
 ...
 * @returns SQL查询字符串
 */
export function generateEarnedPremiumQueryV2(
  statMonth: string,
  _config: NewEarnedPremiumConfig = {}
): string {
  return `
SELECT
  org_level_3,
  ...
  ROUND(total_premium, 2) AS total_premium,
  ROUND(total_fee, 2) AS total_fee,
  ROUND(avg_fee_rate * 100, 2) AS fee_rate,
  ROUND(avg_line_factor, 2) AS line_factor,
  ROUND(first_day_part, 2) AS first_day_part,
  ROUND(time_part, 2) AS time_part,
  ROUND(earned_premium, 2) AS earned_premium,
  CASE
    WHEN total_premium > 0
    THEN ROUND(earned_premium * 100.0 / total_premium, 2)
    ELSE 0
  END AS earned_ratio
FROM EarnedPremiumMonthly
 ...
 * @returns SQL查询字符串
 */
export function generateEarnedPremiumSummaryQueryV2(
  statMonth: string,
  _config: NewEarnedPremiumConfig = {}
): string {
  return `
WITH aggregated AS (
  SELECT
    ...
    SUM(total_premium) AS total_premium,
    SUM(total_fee) AS total_fee,
    SUM(first_day_part) AS total_first_day_part,
    SUM(time_part) AS total_time_part,
...
SELECT
  ...
  CASE
    WHEN total_premium > 0
    THEN ROUND(total_fee * 100.0 / total_premium, 2)
    ELSE 0
  END AS avg_fee_rate,
  ROUND(total_first_day_part, 2) AS total_first_day_part,
  ROUND(total_time_part, 2) AS total_time_part,
  ROUND(total_earned_premium, 2) AS total_earned_premium,
  CASE
    WHEN total_premium > 0
    THEN ROUND(total_earned_premium * 100.0 / total_premium, 2)
    ELSE 0
  END AS earned_ratio
FROM with_totals
ORDER BY
