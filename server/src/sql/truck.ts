 ...
 * @returns SQL查询字符串
 */
export function generateTonnageRoseQueryV2(
  metric: 'premium' | 'count',
  whereClause: string = '1=1',
  perspective: ViewPerspective = 'premium'
): string {
  const agg = metric === 'premium' ? 'SUM(total_premium)' : 'SUM(policy_count)';
  const perspectiveConditions = generatePerspectiveWhere(perspective, [
    whereClause,
    "customer_category LIKE '%货车%'",
  ]);
...
/**
 * V2版本：按吨位分段的机构分析（使用预聚合表，性能提升25-40倍）
 *
 * @param whereClause - WHERE子句条件（默认为 '1=1'）
 * @param perspective - 视角类型（默认 'premium'）
 * @returns SQL查询字符串
 */
export function generateOrgByTonnageQueryV2(
  whereClause: string = '1=1',
  perspective: ViewPerspective = 'premium'
): string {
  const perspectiveConditions = generatePerspectiveWhere(perspective, [
...
/**
 * V2版本：按机构的吨位分段分析（使用预聚合表，性能提升25-40倍）
 *
 * @param whereClause - WHERE子句条件（默认为 '1=1'）
 * @param perspective - 视角类型（默认 'premium'）
 * @returns SQL查询字符串
 */
export function generateTonnageByOrgQueryV2(
  whereClause: string = '1=1',
  perspective: ViewPerspective = 'premium'
): string {
  const perspectiveConditions = generatePerspectiveWhere(perspective, [
...
/**
 * V2版本：三级机构保费占比查询（使用预聚合表，性能提升25-40倍）
 *
 * @param whereClause - WHERE子句条件（默认为 '1=1'）
 * @param perspective - 视角类型（默认 'premium'）
 * @returns SQL查询字符串
 */
export function generateOrgPremiumRatioQueryV2(
  whereClause: string = '1=1',
  perspective: ViewPerspective = 'premium'
): string {
  const perspectiveConditions = generatePerspectiveWhere(perspective, [
