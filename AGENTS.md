...
1. 根地址可达：`GET https://chexian.cretvalu.com/` 返回 `200`
2. 服务健康可达：`GET https://chexian.cretvalu.com/health` 返回 `200`
3. 认证链路可达：`POST /api/auth/login` 返回 `200`
4. 至少一个核心业务 API 可达：例如 `GET /api/query/kpi?...` 返回 `200` + 非空 JSON
5. 至少一个真实浏览器场景通过：登录后进入目标页面并确认核心接口 `200`

**硬规则 2：最终 SHA 发布规则**
- 若在发布过程中发生 `commit` / `push` / `rebase` / `merge`，必须基于最终远端分支 HEAD 重新执行一次完整发布。
- 禁止以“之前已经部署过本地版本”为由跳过最终 SHA 的重新发布。

**硬规则 3：发布脚本健康检查规则**
- 所有 VPS 发布脚本禁止使用固定 `sleep N` 作为唯一健康判断。
...
**执行要求**：输出中必须显式展示上述 4 级证据；任一级缺失，都不得写“已完成部署”。
...
- 声称“生产已恢复/网站访问正常”前，必须额外展示 `/`、`/health`、登录、至少一个核心业务 API、至少一个真实浏览器场景这 5 项证据
- 若发布期间发生新的 `commit/push/rebase/merge`，必须基于最终远端 HEAD 重新发布并重新验收，禁止复用旧证据
...
**变更历史**：
- 2026-03-20：新增 §0.3「生产完成定义与发布硬门禁」，强制 `/` + `/health` + 登录 + 核心业务 API + 浏览器场景五层证据，并要求 merge 后基于最终 SHA 重新发布
