...
description: 出险率发展三角形分析 — 按客户类别/燃料/险别等维度，构建等发展天数出险率+案均+赔付率三角形，支持同比和趋势判断。Use when 用户说"出险率发展/出险率三角形/等发展天数对比"，或需要判断出险率变化是真恶化还是成熟度差异时。
version: 1.0.0
---
...
- 赔案范围：报案时间(report_time)在观察窗口截止点之前 & 保单在窗口内。已结案(settlement_time<观察点)取已决赔款(settled_amount)，未结案取立案金额(reserve_amount)；金额剔除无责(liability_ratio=0)与零结/注销/拒赔（对齐项目 SSOT），件数不过滤
