...
执行前先按 `/diagnose-router` 分流；本命令只处理**过户车 + 车牌归属地 vs 实际出险地**的风控/欺诈专项。

若只是普通事故地点、事故原因或大案下钻，使用 `/diagnose-segment`；若要看机构整体经营亏损，使用 `/diagnose-agent`。本命令输出是风险信号，不直接替代经营利润结论。
