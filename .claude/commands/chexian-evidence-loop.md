...
- **🛡 闸-2（完成对抗·默认关闭，仅本次显式要求才跑）**：实现 + 确定性闸（verify:full/governance）绿后、**enable --auto 前** → **直接调 codex CLI**（不经 skill）审 **diff 完成质量** —— **code review 单源 = codex CLI**（2026-06-25 用户指令）。跑了则 P0/P1 全修 + 复审通过才合并，计入 `codex_done`。**默认（未跑 codex）时** code review 由 `/code-reviewer` 自审兜底，合并凭其通过 + 确定性闸 + CI 双绿 + 非部署链。**跑 codex 时不另叠 `code-reviewer`（codex 单源）、始终不起 `evidence-verifier`，也不把 CI auto-review 计作闸源**；correctness 由确定性闸（verify:full/governance）承担（正交）。
