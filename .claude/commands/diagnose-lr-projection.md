...
- 业务介入预测：通过 `lr_projection_overrides.csv` 提供细分单元级 expected_lr，强制覆盖兜底值

> **报告产物语言红线**：生成的 Markdown 报告正文与板块标题必须用中文（兜底降维/业务介入覆盖/细分单元/满期赔付率），英文仅允许出现在 CSV 列名等技术字段处，首次出现须附中文释义。
...
1. **方法论**：4 维定义 + 兜底降维规则 + 业务介入覆盖应用情况
2. **整体预测**（结论先行）：预测年全年预期满期赔付率（含/不含业务介入覆盖双口径 + 与历史对比）
3. **按客户类别 11 行**（含四级亮灯 🟢🔵🟡🔴）
4. **按能源类型 2 行**（燃油/新能源）
5. **按标准四分类 4 行**（新车/旧车过户/旧车非过户续保/旧车非过户转保）
6. **按险别组合 3 行**（主全/交三/单交）
7. **前 10 名高赔付率细分单元**（最大风险点，预测年满期保费 ≥ 50 万）
8. **前 10 名低赔付率细分单元**（机会点，预测年满期保费 ≥ 50 万）
9. **兜底降维情况统计**（原始四维 / 三维/二维/一维兜底 / 全局兜底 / 业务介入覆盖 的计数与保费占比；对应技术字段 `4d_original` / `*_fallback` / `overall` / `override`）
10. **方法论局限性**与使用建议
