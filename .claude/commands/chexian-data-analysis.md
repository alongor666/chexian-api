...
| 子命令 | 功能 | 使用场景 |
|--------|------|---------|
| `/chexian-data-profile` | 数据概览与质量检查 | 首次分析数据时 |
| `/chexian-data-kpi` | 业绩分析与排名 | 查看业务员/机构业绩 |
| `/chexian-data-trends` | 时间趋势分析 | 分析环比增长和异常 |
| `/chexian-data-export` | 数据导出 | 导出分析结果 |
| `/chexian-cost-analysis` | 成本深度审计 | 赔付率/费用率分析 |
...
```bash
/chexian-data-analysis                           # 默认：全量分析
/chexian-data-analysis --dimensions kpi,trend    # 指定维度
/chexian-data-analysis --period 2025-12          # 指定月份
/chexian-data-analysis --org 成都市本级           # 指定机构
```
...
| # | 维度 | 说明 | 对应子命令 |
|---|------|------|-----------|
| 1 | **数据概览** | 总记录数、字段完整性、基础统计 | `/chexian-data-profile` |
| 2 | **KPI 汇总** | 保费/件数/件均、环比增长、同比对比 | `/chexian-data-kpi` |
| 3 | **机构分析** | 机构排名、保费占比、人均产能 | `/chexian-data-kpi` |
| 4 | **业务员排名** | Top 30 排名、四象限分类、贡献度分析 | `/chexian-data-kpi` |
| 5 | **时间趋势** | 月度/周度趋势线、环比增长率、异常检测 | `/chexian-data-trends` |
| 6 | **险种结构** | 险别组合分布、险种保费占比、件均对比 | — |
| 7 | **续保分析** | 续保率、可续保率、续保潜力、不可续原因 | — |
| 8 | **营业货车** | 吨位分段统计、堆叠柱状图、下钻分析 | — |
| 9 | **成本分析** | 赔付率、费用率、综合费用率、变动成本率 | `/chexian-cost-analysis` |
