...
面向分公司管理者的 6 张三级机构窗口对照表，**以数据截止日当天所在月/年为窗口，忽略 `--time-view`/`--start`/`--end`**：
...
输出 `数据管理/数据分析报告/续保分公司视角_{年}年{月}月_{ts}.md`，6 张表均以 `三级机构 + 应续件数` 起列（表一为经营盯盘主表，额外含未报价/流失/续保影响度）：

| # | 表 | 窗口 | 字段 |
|---|---|---|---|
| 一 | 当月已到期续保表 | 当月到期日 ≤ 数据截止日（已成熟，续保率亮灯） | 应续/已报价/已续保 + **未报价/流失件数 + 续保影响度** + 报价率/续保率；**按续保影响度降序** + 内嵌指标口径映射表 + 两段问题式结论（续保率缺口 / 未报价即流失） |
| 二 | 当月未到期续保表 | 当月到期日 > 数据截止日（续保率反映进度，不亮灯） | 应续/已报价/已续保件数 + 报价率 + 续保率 |
| 三 | 当月续保表 | 当月全部（= 一 + 二） | 同二 |
| 四 | 当年续保表 | 当年全部 | 同二 |
...
- **续保影响度（表一专项注册指标，用户 2026-06-07）= 流失件数 ÷ 合计应续件数**（先聚合后计算，什么分类就按什么合计；可加和，各机构之和 = 整体续保缺口 = 1 − 续保率）。未报价件数 = 应续 − 已报价；流失件数 = 应续 − 已续保（含未报价 + 已报价未成交）。定义单一事实源 `renewal_common.MATURED_GLOSSARY`，表一内嵌口径映射表防漂移。表一按续保影响度从高至低排序，结论只谈问题、每个问题一段。**续保影响度是负面指标**（流失越多越坏），措辞用"导致整体续保缺口扩大"，禁用"贡献"（中文语境贡献为褒义）。
- **server 注册（未来 API 消费）**：三指标已注册进 server——`server/src/sql/renewal-tracker.ts` 主查询输出 D=未报价(A−B)/E=流失(A−C) 件数；`metric-registry` 登记 3 个 L4 占位符（`renewal_unquoted_count`/`renewal_lost_count`/`renewal_impact_rate`，真实 SQL 在续保生成器/本诊断脚本）。⚠️ **未到期窗口的「流失/续保影响度」语义不成立**（未到期非真流失，仅为"待续"），表二/三/四复用时须改语义，勿照搬表一。

### 三级机构视角模式（`--org-report`）— 三级机构续保诊断模板

面向**三级机构管理者**：锁定单一三级机构（`--org` 必填），与分公司视角同样 6 张窗口表、同套口径与亮灯，仅两点不同：分组维度由「三级机构」改为「业务员」；6 张表统一展示**当月应续 top10 固定同一批业务员**（便于横向追踪同一业务员跨窗口表现）。**本模式定位为三级机构续保诊断模板**（用户 2026-06-07）。

```bash
# 高新三级机构 · 6 月续保诊断（业务员视角）
python3 数据管理/pipelines/diagnose_renewal.py --org-report --org 高新

# 可叠加团队范围与进盘提前期
python3 数据管理/pipelines/diagnose_renewal.py --org-report --org 天府 --team 天府业务一部 --pool-lead-days 30
```

输出 `数据管理/数据分析报告/续保三级机构视角_{机构}_{年}年{月}月_{ts}.md`。6 张表结构、字段、亮灯、表一三指标（未报价 / 流失 / 续保影响度）与结论范式全部与分公司视角一致（见上表），仅维度列为「业务员」。

**与分公司视角的差异（仅此三点，其余完全一致）**：

| 维度 | 分公司视角（`--branch-report`） | 三级机构视角（`--org-report`，模板） |
|------|------|------|
| 范围 | 全部三级机构 | 锁定单一三级机构（`--org` 必填） |
| 分组列 | 三级机构 | 业务员 |
| 展示行 | 全部三级机构 | **当月应续 top10 固定同一批业务员**（6 表同一批，便于跨窗追踪） |
| 合计行 | 全部三级机构之和 | **该机构全部业务员的真实整体**（≠ top10 之和；top10 各项之和 < 合计） |

**口径要点**：
- **固定 top10 选定**：按**当月**应续（去重车架号）降序取前 10（与小机构阈值 `SMALL_ORG_SALESMEN` 同源），6 张表统一展示这同一批人。各表内部排序仍同分公司视角（表一按续保影响度降序，二~六按各窗口应续降序）。
- **续保影响度可加和性不变**：分母为「该机构全部业务员的合计应续」（真实整体），故各业务员续保影响度之和 = 整体续保缺口；所列 top10 之和 < 整体缺口（其余业务员计入合计未单列，表头自动注明全部业务员数与未单列数）。
- **交叉一致性锚点**：表一（已到期）+ 表二（未到期）= 表三（当月）应续合计；表一合计应续 = 源数据直查「该机构当月已到期去重车架号」。

> **实现复用**：`--org-report` 与 `--branch-report` 共用 `diagnose_renewal_branch.py` 同一套 `_branch_matured_section` / `_branch_funnel_section` / `_branch_speed_section`（参数化 `dim_col` / `dim_header` / `keep_dims`），口径只定义一次、杜绝漂移。新增三级机构诊断需求时套此模板即可。
