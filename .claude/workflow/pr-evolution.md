...
> `/commit-push-pr` 执行前必读此文件，将已记录的失败模式纳入前置检查。
...
- **根因**: 为什么现有检查没拦住
- **修复**: 本次怎么修的
- **预防**: 加入了什么新检查（写到 /commit-push-pr 的哪一步）
```
...
- **根因**: AI 手动执行 git push，没有调用 `/commit-push-pr`；无 hook 拦截
- **修复**: 手动推送后补建了 PR
...
- **修复**: 补建第二个 commit
- **预防**: `/commit-push-pr` 步骤 1 新增「依赖链完整性检查」——修改前端 API 调用时，必须 grep 确认后端路由、paths、api-routes 三处均已注册
