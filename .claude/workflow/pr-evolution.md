...
## 2026-07-12 · 生产"既有备份"跑了 34 天却 0 份业务数据——扁平通配假安心的生产实证

- **场景**：W1a 上生产部署 warehouse 备份时发现 VPS 已有 /root/backup-chexian.sh 每天 02:00 跑（34 个日目录），但其 `cp /var/www/chexian/server/data/*.parquet` 顶层通配匹配不到任何子目录 parquet——34 天"备份"合计 5.2M，全是顶层 json/.env，业务数据 0 份。审计 FIND-002 的"扁平通配漏文件"不是理论风险，是生产在跑的假安心。
- **根因**：手抄脚本从文档 heredoc 装机后无人验证"备出来的东西能不能还原出数据"；备份只看"任务在跑"，不看产物内容。
- **修复**：新备份脚本以"还原对账"为验收（152 文件 sha256 全一致才算数）；旧任务保留（顺带备 .env/顶层 json，无害）。
- **预防**：任何备份类机制上线必须当场做一次"从产物完整还原+对账"，"定时任务存在"不构成备份存在的证据。

### needs_automation: false
（warehouse-backup.sh 的 verify 模式即自动化对账工具；可选 follow-up：看门狗式月度自动 verify。）

---

## 2026-07-12 · 当日发布被 governance USER_PASSWORDS 闸拦停——env 残留同类第 2 次（worktree→主仓）

- **场景**：指挥中枢第三轮期间发现当日自动发布 2 次失败停手、生产数据停 07-11。launchd 日志定位：release:daily 的主仓 governance 前置闸 54/55 过，唯一红项=主仓 server/.env USER_PASSWORDS 残留 6 个自助设密账号。07-11 的 pr-evolution 复盘已预警"主仓若同样残留也会红"，预警成真。
- **根因**：#1068 阶段二交接清单的"主仓/生产 VPS env 清理"两项没有卡片化闭环，散在 PR 描述里被遗忘；闸按设计工作（防的是密码注入旁路），代价是发布停摆。
- **修复**：修复动作（改凭据库）被会话权限闸判定超出授权、待用户批准；已备份 .env、开 P1 事故卡 c0f97a 全量记录处置路径。
- **预防**：① 交接清单类残留任务必须逐项开 backlog 卡而非停留在 PR 描述（本次已补开 c0f97a）；② 同类 env 残留已两次，若批准修复后第三处（VPS 侧）仍残留，governance 报错信息应附一键修复命令并在 #1068 系列收尾时全环境扫一遍。
