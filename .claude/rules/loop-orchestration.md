...
> **跑 codex 时不另叠 `code-reviewer`（codex 单源）；始终不起 `evidence-verifier`，也不把 `claude-code.yml` CI auto-review 计作闸源**（CI job 仍可在 PR 上空跑，但不作为闸-2 的判定源）。默认（未跑 codex）时 code review 由 `/code-reviewer` 兜底（见上方默认状态）。
