...
`scripts/auto-release-daily.mjs`**（无常驻进程），在北京时间窗口（默认 10:35~20:00）内
...
当天 `released` → 全天幂等跳过；`failed` 窗口内重试至上限（默认 6 次）后停手等人工；
...
环境变量：`AUTO_RELEASE_WINDOW_START/END`（北京时间，默认 10:35/20:00）·
`AUTO_RELEASE_MAX_ATTEMPTS`（默认 6）·
