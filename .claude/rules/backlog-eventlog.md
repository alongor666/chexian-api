...
## 12. 每事件一文件：根治 GitHub 服务端 PR 冲突（2026-07-11 · 卡 637c35）

> 来源：BACKLOG `2026-07-10-claude-637c35`。§10 让 PR 只碰真相日志后，`BACKLOG_LOG.jsonl` 单文件尾部追加仍是全仓最高并发写竞争点——卡链「上一卡建 PR 即 spawn 下一卡」使相邻 PR 基线互不含对方追加行，两 PR 均改同一文件尾。`.gitattributes` 的 `merge=union` **只在本地 rebase/merge 生效，GitHub 服务端计算 PR 可合并性不应用 merge 策略** → PR 页仍标 CONFLICTING、CI 不入队（memory `ci-zero-workflow-runs-means-pr-conflict` 同源）。本节把这最后一处冲突源结构性消除。

**模型**：

| | 存量 | 增量 |
|---|---|---|
| 载体 | `BACKLOG_LOG.jsonl`（**冻结只读**，2026-07-11 起禁止追加） | `backlog-events/<YYYY-MM>/<at压缩>-<eid>.json`（**每事件一个独立文件**） |
| 写入 | ——（纠错也走新目录追加事件） | `scripts/backlog/lib.mjs appendEvents()`（唯一写路径，`backlog.mjs` 各子命令经它写入；flag `wx` 撞名即响亮失败） |
| 读取 | `loadLog()` 合并两源（jsonl + 目录），fold 按 `(at, eid)` 全序折叠，**与事件物理落位无关** | 同左 |

**为什么按月分片**：单目录数千文件会让该目录的 git tree 对象每次提交整体重写；按 `at` 的 `YYYY-MM` 分片把单目录条目数限在月事件量（历史峰值约 700），tree 抖动与 readdir 成本均有界。

**实施前评估（卡内要求，2026-07-11 实测）**：
- **读性能**：20000 个事件文件（约 2.5 年事件量）readdir + 逐文件读 + JSON.parse 中位 **289ms**（等量单 jsonl 8.5ms）；增量目录从 0 起步，一年量级（约 8000 文件）约 115ms，CLI 可接受。
- **git 对象**：700 提交（每提交 1 事件）gc 后 `.git`：每事件一文件 **1052KB** vs jsonl 追加 **4516KB**——对象数 +67%（3503 vs 2103）但 pack 后体积反而更小（小 blob + tree delta 压缩优于大文件快照链）。两项均不构成否决。

**迁移选择 = 冻结只读**（非一次性拆分）：拆分会产生一次性巨型 diff 且要求所有在飞分支同步适配；冻结让存量在飞分支的旧式 jsonl 追加经 union 正常并入、读取层照样可见，**零断裂过渡**。jsonl 的 `merge=union` 属性保留兜底，待在飞分支清空后即为纯历史。

**铁律增补**（其余铁律不变；§2「日志行禁原地改」对目录事件文件同样成立——文件一经写入禁改禁删，纠错走追加新事件）：
- ❌ 禁止再向 `BACKLOG_LOG.jsonl` 追加事件（写入方只有 `appendEvents()` 一条路，`backlog.mjs` 已改）
- ❌ 禁止手工创建/改名/复制事件文件——文件名必须为 `<at压缩>-<eid>.json` 且与内容 eid 一致（governance「BACKLOG事件日志」新增文件名↔eid 一致性守卫）
- 跨 ref 读取（如 dispatch 认领扫描）：jsonl 用 `git show <ref>:BACKLOG_LOG.jsonl`，目录用 `git grep -h -e "" <ref> -- backlog-events`（单行 JSON 文件 → 一次子进程倾倒该 ref 全部目录事件）

**收口纪律降级**：loop-orchestration §4 2026-07-10 meta 的「jsonl 追加放收口最后一步 + 等前卡 PR 合并后再 push」是旧结构下的规避手段；本节落地后不同 PR 天然写不同文件，该纪律不再必要（保留无害）。
...
- 每事件一文件写读层：[../../scripts/backlog/lib.mjs](../../scripts/backlog/lib.mjs)（appendEvents/loadEventsDir/eventFilePath，§12）
- AGENTS.md §8.2 append-only：本文件作为新增独立护栏文件，无需 `[policy-override]` 授权；§8/§10/§11/§12 为新增小节（append），非改写既有内容
