...
记录数: ~620,000 条
字段数: 38 个
核心维度: 业绩、续保、产品、渠道、客户、批改、成本、交叉销售、风险等级
```
...
必填字段 (14个):
  ...
  - 属性: 险类、险别组合、客户类别
  - 标识: 是否续保/新车/过户/新能源/电销
  - 金额: 保费

可选字段 (24个):
  - 赔付费用: 赔案件数、已报告赔款、费用金额
  - 交叉销售: 交叉销售标识、交叉销售保费_驾意
  - 风险: 车险风险等级 (COALESCE三字段合并, A-G/X)
  - 保额: 三者保额、司机保额、乘客险保额
  - 续保: 续保单号、续保模式
  - 日期: 提核日期
  - 其他: 车牌号码、座位数、吨位分段、终端来源、车架号、厂牌车型
  - 商业: 商车自主定价系数、是否交商统保、新车购置价
  - 批改: 批单号、批改类型、是否报价、是否可续
```
...
```bash
# 方式1: 读取JSON (机器可读)
cat 数据管理/knowledge/schema/schema-analysis.json

# 方式2: 读取Markdown (人类可读)
cat 数据管理/字段字典.md

# 方式3: 读取业务规则 (唯一事实源)
cat 数据管理/knowledge/rules/车险数据业务规则字典.md

# 方式4: 读取关联分析 (深入理解)
cat 数据管理/knowledge/ai/DATA_FLOW_KNOWLEDGE.md
```
...
参考:
- 数据管理/knowledge/rules/车险数据业务规则字典.md § "保费字段"
- 负保费处理规则: 保费<0为批改退费
...
参考:
1. 数据管理/knowledge/rules/车险数据业务规则字典.md § "客户类别" + "吨位分段"
2. 数据管理/knowledge/ai/PARQUET_SCHEMA_KNOWLEDGE.md § "营业货车专项模式"
...
快速参考:
- .claude/data-knowledge-protocol.md § 第1层快速索引
