...
**变量（应消除硬编码）= A+B ≈ 51 处**；**通用（保持不动）= C ≈ 30 处**；**灰色待业务确认 = D ≈ 8 处**。
...
- `generate_dim_tables.py` 业务员/计划维度表只有四川 xlsx → **SX salesman/plan 维度表为空** → 依赖 JOIN 的路由（performance-drilldown/bundle）Binder Error 空 body。
...
**结论：当前架构新增一省 ≈ 改动 20+ 处、跨 4 链路、含多个静默失效陷阱。这是核心架构债。**
