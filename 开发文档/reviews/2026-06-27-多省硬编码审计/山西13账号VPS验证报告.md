...
| P1 | **报价转化页 + 客户来源页对 SX schema 不匹配** | `/quote-conversion/*`、`/customer-flow/*` 报 `列不存在：policy_date`。SX 数据域缺 `policy_date` 列或 SQL 用了四川专有列名。整页系不可用。 | 数据/ETL schema |
...
- **"慢路由 12–14s"** → 假象。限流期并发轰炸 + 429 排队所致；单请求复测 sxadmin 全省 `claims-detail/pending-overview` 冷 0.28s/热 0.12s、`comprehensive-bundle` 冷 0.58s/热 0.21s，性能正常。
