...
**结论**:38→39 是 PR #355 时点(2026-05-11)的历史快照真实观测,机制属于"边界 cell 因去重导致 earned_premium 跨阈值漂移",非脚本 bug。该项作为历史快照保留,**PR-0.5 / PR-1 不会重新精确复现此具体数字**;但 `HAVING SUM(premium) > 0` 对边界 cell 的影响机理已通过当前数据下的绝对数对账(§1.1,赔款减少 5,046 万、满期保费几乎不变)间接验证。
...
- **PR #358**:follow-up 轮 2(P1-1 历史端绝对数 + P1-3 38→39 反直觉机制说明)— 合入 main 提交 `be8d4de`
- **PR-0.5**(本次):移除 `--debug-hardening-stage` 临时开关 / `HARDENING_STAGES` 常量 / 一次性脚本 `scripts/lr_baseline_diff.py` / 失意义阶段对比测试 — 完成 B287 阶段 0 主线收尾
- **遗留任务**:PR-1(薄模块化拆分,按需推进)
