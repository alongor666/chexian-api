...
| `components/` | layout/ | 全局布局组件（SidebarLayout / TopNavigation / ErrorBoundary 等） | 活跃 |
...
| `workflows.ts` | `/api/workflows` | 工作流调用（驱动 `server/src/skills/workflows/`） |
...
| `performance-analysis-shared.ts` | 业绩分析域共享逻辑（barrel → `server/src/sql/performance-analysis/shared.ts`） |
...
> 无顶层 barrel 的子目录：`cube/`（通用立方体查询加速，消费方含 `server/src/services/duckdb-cube.ts`、`server/src/sql/kpi.ts` 与多个 `routes/query/*` 子路由）、`forecast/`（利润预测基线，消费方 `server/src/agent/services/agent-forecast-baseline-service.ts`）、`shared/`（跨域共享片段：`business-conditions.ts` / `policy-dedup.ts`）。行数/文件数不在本表登记，以目录实际为准。
...
| 文件 | 职责 |
|------|------|
| `duckdb-cube.ts` | 立方体物化与查询执行（消费 `server/src/sql/cube/`；构建失败同版本退避 3 次 + OOM 结构化降级，2026-07-09 性能审计） |
...
| 文件 | 职责 |
|------|------|
| `auth.ts` | 认证服务（JWT 生成、密码验证、三级哈希链、pns 判定、设密/改密） |
...
| `activation-token.ts` / `activation-token-store.ts` | 激活令牌（cx_act_ 一次性 24h 设密凭据，管理员签发；单层存储 sqlite/json 双 backend） |
...
| `adapters/` | `query-adapter.ts` + `server/src/skills/adapters/llm/`（LLM 适配） |
| `types.ts` | 技能/工作流类型定义 |
...
| 文件 | 职责 |
|------|------|
| `server/src/config/field-registry/fields.json` | **字段唯一事实源**（字段数以该文件为准），codegen: `node scripts/field-registry/generate.mjs` |
...
| 类型/接口 | 路径 | 说明 |
|-----------|------|------|
| `ColumnMapping` | `server/src/normalize/mapping.ts` | 列名映射类型（字段数以 `server/src/config/field-registry/fields.json` 为准，codegen 派生，支持多别名） |
...
- 修改禁止文件：必须先经 `bun scripts/backlog.mjs` 在需求账本（`BACKLOG_LOG.jsonl` + `backlog-events/`）登记并提供证据链。
...
- `src/features/dashboard/hooks/useDashboardData.ts`、`src/features/dashboard/Dashboard.tsx`、`src/features/filters/FilterPanel.tsx` 已归档并于后续迭代物理删除（原归档目录 `archive/legacy-code/2026-02-api-only/` 已清理，历史见 git）。 <!-- governance-allow: index-doc-links 墓碑说明，所列路径为已删除载体 -->
- 当前看板主链路入口：`src/features/dashboard/PremiumDashboard.tsx` + `src/features/filters/PageFilterPanel.tsx`。
- 类型检查护栏：`scripts/check-governance.mjs` 新增 `TS检查范围`，禁止通过 `tsconfig.exclude` 排除活跃目录规避类型问题。

**变更记录**：
- 2026-07-16：知识体系审计死链清理（详见 `开发文档/审计/2026-07-16-知识体系审计.md`）——裸 `BACKLOG.md`（gitignored 派生视图）引用改事件账本口径、已清理的 `archive/legacy-code/` 引用改墓碑、`PageFilterPanel.tsx` 路径修正为 `src/features/filters/` <!-- governance-allow: index-doc-links 变更记录墓碑 -->
